...
namespace Login
{
    public class D_Aplicacion
    {
        ...
        public static E_Aplicacion Consultar(E_Aplicacion eproveedor)
        {
            ...
            try
            {
                OdbcCommand busqueda = new OdbcCommand(
                    string.Format("SELECT ncodproveedor AS 'Codigo proveedor',  FROM maproveedor WHERE ncodproveedor = '{0}'", eproveedor.Idproveedor),
                  ConexionODBC.Conexion.ObtenerConexion()
                );
        ...
        public static DataTable buscarproveedor() // consulta para el grid de roles
        {
            ...
            try
            {
                mySqlComando = new OdbcCommand(
                     string.Format("SELECT ncodproveedor, vnombre FROM MaPROVEEDOR"),
                     ConexionODBC.Conexion.ObtenerConexion()
                 );
