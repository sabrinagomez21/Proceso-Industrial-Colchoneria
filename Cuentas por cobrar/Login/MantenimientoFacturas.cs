...
/*---------------------------------------------------------------------------------------------------------------------------
    ...
    ANALISTAS Y PROGRAMADORES: JOSUE REVOLORIO
 ---------------------------------------------------------------------------------------------------------------------------*/
...
namespace Login
{
    public partial class MantenimientoFacturas : Form
    {
        ...
        public MantenimientoFacturas()
        {
            ...
            cnegocio.funactivarDesactivarCombobox(cmbCliente, false);
        }
        ...
        {
            clasnegocio cnegocio = new clasnegocio();
            cnegocio.funconsultarRegistros("mafacturacobro", "SELECT ncodfactura, nnofactura, dfecha, dfechaexpiracion, ntotal, cpagado, ncodcliente from mafacturacobro WHERE cestado='ACTIVO'", "consulta", grdFacturas);
            cnegocio.funconsultarRegistrosCombo("ncodcliente", "SELECT concat(ncodcliente, '.', vnombrecliente) as nombre from maclientecobro WHERE vestado='ACTIVO'", "nombre", cmbCliente);
        }
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            if(sBanderaGuardar == "Guardar"){
                ...
                txtCliente.Text = funCortadorID(cmbCliente.Text);
            ...
            dtpFecha.Enabled = dtpVencimiento.Enabled = false;
            cn.funactivarDesactivarCombobox(cmbCliente, false);
        }
            ...
            if (bBuscando == true)
            {
                try
                {
                    if (!String.IsNullOrEmpty(cmbCliente.Text))
                    {
                        clasnegocio cnegocio = new clasnegocio();
                        txtCliente.Text = funCortadorID(cmbCliente.Text);
                        cnegocio.funconsultarRegistros("mafacturacobro", "SELECT ncodfactura, nnofactura, dfecha, dfechaexpiracion, ntotal, ncodcliente from mafacturacobro WHERE cestado='ACTIVO' AND dfechaexpiracion LIKE '" + txtCliente.Text + "%'", "consulta", grdFacturas);
                    }
                    else
                    {
                        funActualizarGrid();
