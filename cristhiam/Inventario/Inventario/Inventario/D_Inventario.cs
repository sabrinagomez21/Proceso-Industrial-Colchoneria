...
namespace Inventario{
    ...
    public class D_Inventario : CAD
    {
        ...
        public static int Actualiza(E_Inventario eInventario)
        {
            ...
            {//Conexion y Inserccion
                mySqlComando = new OdbcCommand(
                string.Format("UPDATE inventario SET cantidad = '" + eInventario.cantidad + "',  descripcion = '" + eInventario.descripcion + "' WHERE producto = '" + eInventario.producto + "';"), CAD.ObtenerConexion());
                iValorRetorno = mySqlComando.ExecuteNonQuery();
            }
            catch (Exception e)
            {
